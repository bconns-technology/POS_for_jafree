...
<hr style="margin-bottom: 0px;">
  		...
  		<h1 style="text-align: center;">Daily Invoice</h1>
  		<hr style="width: 30%;">
  		<br>
	    ...
	    <table border="3" class="table-responsive" width="100%">
	    	<thead >
	 		 ...
	 		 <tbody>
			    ...
			    <tr>
					<td>{{$key+1}}</td>
					<td>{{$d['payment']['customer']['name']}}			
							({{$d['payment']['customer']['phone']}}-{{$d['payment']['customer']['address']}})</td>
					<td >Invoice No#{{$d->invoice_number}}</td>
					<td >{{date('d-m-Y', strtotime($d->date))}}</td>
					<td >{{$d->description}}</td>
					<td class="text-center" >	{{$d['payment']['total_amount']}}	
						
					</td>
					@php
	 		 	$total_sum+=$d['payment']['total_amount'];
	 		 	@endphp
